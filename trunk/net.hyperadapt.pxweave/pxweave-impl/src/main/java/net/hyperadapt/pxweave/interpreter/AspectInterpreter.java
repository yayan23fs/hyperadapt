...
import net.hyperadapt.pxweave.aspects.ast.EvaluationType;
...
public class AspectInterpreter extends Logable implements IAspectInterpreter {
	...
	private HashMap<String, IInterpreterArgument> initialiseArguments(
		...
		for (final Aspect aspect : aspects) {
			...
			URI namespaceURI = URI.create(aspect.getInterface().getCore()
					.getType());
			URI schemaURI = environment.getNamespaceContext().getDefinitionURI(
					namespaceURI);
			for (final IInterpreterArgument interpreterArgument : interpreterArguments) {
				if (interpreterArgument.getId().contentEquals(core)) {
					...
					if (!arguments.containsKey(core)) {
						...
						if (!arguments.get(core).getSchemaURI()
								.equals(schemaURI)) {
							throw new XMLWeaverException(
									"Core \""
											+ aspect.getInterface().getCore()
													.getId()
			...
			if (!coreFound) {
				throw new IllegalArgumentException("Core \""
						...
						+ "\" not found in runtime context."
						+ " Core was declared in aspect \"" + aspect.getName()
						+ "\"");
			}
		}
	...
	private IXQAbstractArgument initialiseXQueryContext(Aspect aspect,
			Document coreDocument, IWeavingContext context)
			throws XMLWeaverException {
		...
		IXQAbstractArgument xqArgument = new XQArgument(coreDocument,
				environment.getNamespaceContext(),
				environment.getNamespaceContext());
		if (aspect.getInterface().getDocumentParameters() != null) {
			final List<DocumentParameter> documentParameters = aspect
					.getInterface().getDocumentParameters()
			...
			for (final DocumentParameter documentParameter : documentParameters) {
				...
				try {
					...
					if (documentURI.isAbsolute()) {
						...
						parameter = parser.buildDOM(environment.getBaseURI()
								.resolve(documentURI));
					}
					...
					throw new XMLWeaverException(
							"Document parameter not bound---aspect name: "
		...
		ContextVarDecl contextVarDecl = aspect.getInterface()
		...
		if (contextVarDecl != null && contextVarDecl.getParameter() != null) {
			for (final ContextParameter para : contextVarDecl.getParameter()) {
				...
				if (value != null) {
					try {
						xqArgument.addVariableAndBindValue(para.getName(),
								value);
	...
	private void loadAndValidateArguments(
		...
		for (final IInterpreterArgument argument : arguments.values()) {
			if (!argument.isLoaded()) {
				...
				argument.loadDocument(parser, environment.getValidationMode()
						.isValidateInput());
			} else {
	...
	private boolean checkContextParameters(Aspect aspect,
			IWeavingContext context) {
		boolean allDeclared = true;
		if (aspect.getInterface().getContextParameters() != null) {
			for (final ContextParameter parameter : aspect.getInterface()
					.getContextParameters().getParameter()) {
				if (context.isMissingValue(parameter.getName())) {
					getLogger()
							.info("Missing parameter '" + parameter.getName()
									+ "' for aspect '" + aspect.getName() + "'");
					allDeclared = false;
				}
			}
		}
	 ...
	 */
	public List<IInterpreterArgument> interprete(
		...
		for (final Aspect aspect : environment.getAspects()) {
			if (!executionState.getCurrentJoinpoint()
					.containsApplicableAdvices(aspect, adviceLocator)) {
				continue;
			}
			getLogger().debug(
					"Aspect '"
							+ aspect.getName()
							+ "' may contain applicable advices ("
							+ adviceLocator.toString()
							+ " '"
							+ executionState.getCurrentJoinpoint()
									.getIdentifier() + "').");
			...
			final IInterpreterArgument coreDocumentArgument = iArguments
					.get(coreId);
			...
			getLogger().debug(
					"Checking if all required context parameters exist ...");
			if (!checkContextParameters(aspect,
					executionState.getContextModel())) {
				throw new XMLWeaverException(
						"Missing context parameters for aspect '"
								+ aspect.getName() + "', see Log for details.");
			}

			getLogger().debug(
					"Initializing context for XQuery based evaluation ...");
			IXQAbstractArgument xqContext = initialiseXQueryContext(aspect,
					coreDocument, executionState.getContextModel());

			final List<AdviceGroup> groups = aspect.getAdviceGroup();
			boolean appliedAdvice = false;
			for (final AdviceGroup adviceGroup : groups) {
				if (!executionState.getCurrentJoinpoint().applies(adviceGroup,
						adviceLocator)) {
					continue;
				}

				try {
					if (isActive(xqContext, adviceGroup)) {
						Document result = interpreteAdviceGroup(adviceGroup,
								aspect, xqContext, coreDocument);

						// TODO check if setting the result is really needed
						final IInterpreterArgument resultArgument = iArguments
								.get(coreId);
					...
					throw new XMLWeaverException(
							"Can't evaluate dependencies [Aspect '"
									+ aspect.getName() + "', reason: "
									+ e.getMessage() + "]", e);
				}
			}
			if (!appliedAdvice)
				getLogger()
						.info("Did not find any active advice applicable to content, document was not modified.");
		}
		if (environment.reportConflicts()) {
	...
	private boolean isActive(IXQAbstractArgument xqContext,
		...
		Dependency dependencies = adviceGroup.getDepends();
		if (dependencies != null && dependencies.getBoolExpr() != null) {
			...
			if (!firstSparQL
					&& (evaluationMode == null || evaluationMode == EvaluationType.ALL)) {
				isActive = evaluateDependency(boolExpr, xqContext);
				if (isActive) {
					isActive = ContextModelHelper.evaluateAdviceCondition(
			...
			if (!firstSparQL && evaluationMode == EvaluationType.EXPRESSION) {
				isActive = evaluateDependency(boolExpr, xqContext);
				if (!isActive) {
					isActive = ContextModelHelper.evaluateAdviceCondition(
							environment.getConfiguration()
			...
			if (firstSparQL || evaluationMode == EvaluationType.SPARQL) {
				isActive = ContextModelHelper.evaluateAdviceCondition(
						environment.getConfiguration()
								.getContextModelEndpoint(), adviceGroup,
						environment.getExecutionState().getContextModel());
	...
	private static boolean evaluateDependency(final String boolExpr,
		...
		boolean isActive = true;
		try {
			xqArgument.setExpression(boolExpr);
			...
			throw new XMLWeaverException("Evaluation of expression \""
					+ boolExpr + "\" does not result in a boolean value");
		}
		return isActive;
	}

	private Document interpreteAdviceGroup(AdviceGroup adviceGroup,
			Aspect aspect, IXQAbstractArgument xqContext, Document coreDocument)
			throws XMLWeaverException {
		final IAdviceContext adviceContext = new AdviceContext(
				adviceGroup.getScope(), aspect.getName());

		AdviceInterpreter adviceInterpreter;
		if (environment.getValidationMode().isValidateOperations()) {
			adviceInterpreter = new AdviceInterpreter(adviceContext,
					coreDocument, xqContext,
					new OperationInterpreter(validator), environment);
		} else {
			adviceInterpreter = new AdviceInterpreter(adviceContext,
					coreDocument, xqContext, new OperationInterpreter(),
					environment);
		}
	...
	private List<IInterpreterArgument> outputArguments(
		...
		if (environment.getValidationMode().isValidateResult()) {
			final SchemaValidator schemaValidator = SchemaValidator
					.getInstance();
			for (final IInterpreterArgument argument : arguments.values()) {
				schemaValidator.documentIsValid(argument.getDocument(),
						argument.getSchemaURI());
			}
		}
