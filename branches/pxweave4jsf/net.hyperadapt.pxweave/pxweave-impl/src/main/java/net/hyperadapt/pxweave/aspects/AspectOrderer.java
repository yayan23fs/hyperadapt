...
public class AspectOrderer {
	 ...
	 * {@link IXMLWeaverException} if the graph can't be built because of errors
	 * in {@link AspectOrder}.
	 * 
	...
	private static DirectedGraph<Aspect, DefaultEdge> builtGraph(
		...
		for (final Aspect a : unorderedAspects) {
			if (a.getInterface().getAspectOrder() != null) {
				...
				if (a.getInterface().getAspectOrder().getAbsolutePosition().value().contentEquals("asLast")) {
					if (!last) {
						last = true;
						lastAspect = a;
	 ...
	 * isn't a DAG this method throws an {@link IXMLWeaverException} and states
