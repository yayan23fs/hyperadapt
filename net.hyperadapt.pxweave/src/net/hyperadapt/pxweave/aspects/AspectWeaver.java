...
/**
 * The AspectWeaver observes the execution state of the environment and reacts on state changes by weaving aspects appropriate to the joinpoint the current  state represents and the current data set consisting of the context model and  input and output parameters.
...
public class AspectWeaver extends Logable implements IExecutionStateObserver{
	...
	/**
	 * Instantiate the weaver by passing a fresh environment. The Weaver will register
	 * itself to the environment as an execution state observer.
	 * 
	 * @param environment - The environment where the weaver is executed.
	 */
	public AspectWeaver(IEnvironment environment) {
		...
		getLogger().info("Aspects will be woven in the order ["+aspectOrderString+"]");
	...
	/**
	 * Invoke the weaver on a specific execution state. The weaver will delegate the 
	 * actual aspect interpretation to an AspectInterpreter.
	 * 
	 * @param executionState - The execution state to invoke the weaver on.
	 * @return Currently the weaver flat copies the data in the execution state, excluding 
	 * 				input and output locations. These copies are returned. 
	 * @throws XMLWeaverException
	 */
	private List<IInterpreterArgument> weave(IExecutionState executionState, AdviceLocator adviceLocator) 
									throws XMLWeaverException {
	 ...
	 * 
	 * @param baseURI - The base URI where the weaver config is located.
	 * @param args - The arguments the weaver should be invoked on, including 
	 * 			input and output files(!).
	 * @throws XMLWeaverException
	 */
	public static void weave(URI baseURI, List<IInterpreterArgument> args) throws XMLWeaverException{
		weave(Environment.create(baseURI),args);
	}
	
	/**
	 * Invoke the weaver statically, e.g., from command line using a self defined 
	 * environment. The weaver will only be executed once at the current joinpoint.
	 * 
	 * @param environment - An initialised environment
	 * @param args - The arguments the weaver should be invoked on, including
	 * 			input and output files.
	 * @throws XMLWeaverException
	 */
	public static void weave(IEnvironment environment, List<IInterpreterArgument> args) throws XMLWeaverException{
		AspectWeaver weaver = new AspectWeaver(environment);
		XMLWriter writer = XMLWriter.getInstance();
		IExecutionState state = new ExecutionState(environment.getExecutionState().getCurrentJoinpoint(),environment.getExecutionState().getContextModel(),args);
		environment.beforeExecutionState(state);
		environment.updateExecutionState(state);
		
		for (final IInterpreterArgument argument : args) {
			writer.writeToFile(argument.getDocument(), argument
					.getOutputFile().getAbsolutePath(), argument
