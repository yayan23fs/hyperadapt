...
/**
 * class for the dynamical analysis to find interference and affectRelation
 * conflicts
 * 
 * @author dkadner
 */
public class DynamicAnalysis extends Logable implements IDynamicAnalysis {

	private List<IAdviceToCheck> advicesWithJP;
	...
	private IAdviceContext adviceContext = null;
	private AdviceLocator adviceLocator = null;

	private boolean logged = false;

	private IConflictMatrix conflictMatrix = null;

	public DynamicAnalysis(IConflictMatrix conflictMatrix) {
		advicesWithJP = new ArrayList<IAdviceToCheck>();
		nsContext = null;
		adviceContext = null;
	...
	public void setPPC(IProgrammaticJoinpoint ppc, AdviceLocator al) {
		...
		getLogger().debug(
				"Current PPC: (" + this.adviceLocator.toString() + ") "
						+ this.ppc.getIdentifier());
		// System.out.println("current PPC: " + this.ppc.toString());
	}

	private List<AffectedAdvicePair> computeAffectRelation() {
		...
		for (int i = 0; i < advicesWithJP.size() - 1; i++) {
			IAdviceToCheck first = advicesWithJP.get(i);
			// NodeList nlFirst = (NodeList) first[1];
			for (int j = i + 1; j < advicesWithJP.size(); j++) {
				IAdviceToCheck second = advicesWithJP.get(j);

				// wenn in Konfliktamtrix, dann untersuchen ob wirklich
				// Konflikt
				if (conflictMatrix.isConflictBetween(first.getBasicAdvice()
						.getClass().getSimpleName(), second.getBasicAdvice()
					...
					String message = "-- Advice "
							...
							+ " (Pointcut: "
							+ first.getBasicAdvice().getPointcut().getValue()
							+ ")"
							+ " INFLUENCE Advice "
							+ second.getBasicAdvice().getClass()
									.getSimpleName() + " (Pointcut: "
							+ second.getBasicAdvice().getPointcut().getValue()
							+ ")";
					...
					indirectNodes = catchIndirectConflict(
							first.getNodesBefore(), second.getNodesBefore());
				...
				if (first.getNodesBefore().getLength() != 0) {
					...
					final String subpath = second.getBasicAdvice()
							.getPointcut().getValue();

					// /Just to handle xpath expressions if no scope is present
					List<String> fullXPathList = new LinkedList<String>();
					if (adviceContext.getScope().getXpath() != null
							&& !adviceContext.getScope().getXpath().isEmpty()) {
						for (final XPath point : adviceContext.getScope()
								.getXpath()) {
					...
					for (final String fullXPath : fullXPathList) {
						...
						try {
							NodeList cpcBOnDoc = xPathEvaluator.evaluateXPath(
									fullXPath, first.getBeforeDocument());
							// anwenden des XPath des 2. Advices auf das after
							// Document des 1. Advices
							NodeList cpcBOnDocWithA = xPathEvaluator
									.evaluateXPath(fullXPath,
											first.getAfterDocument());
							final NodeList joinPoints;
							joinPoints = xPathEvaluator.evaluateXPath(
									fullXPath, first.getBeforeDocument());

							second.setNodesBeforeOtherAdvice(joinPoints);

							if (!sameNodeList(cpcBOnDoc, cpcBOnDocWithA)) {
								AffectedAdvicePair pair = new AffectedAdvicePair(
										first.getCopy(), second.getCopy());
								affectRelation.add(pair);
							}

						} catch (XMLWeaverException e) {
							getLogger()
									.warn("Exception during critical pair comparison occured.",
											e);
						}
	...
	/**
	 * method to decide whether the founded affectRelation advice error has an
	 * inhibit or triggering behavior
	 * 
	 * @param affectedPair
	 *            the advice error to decide for, whether the affectRelation
	 *            behavior is a triggering or inhibition
	 */
	private void checkIfTriggersOrInhibits(AffectedAdvicePair affectedPair) {
		...
		// Unterscheide gefunde affectRelation nach auslösender und hemmender
		// Wirkung:
		// justDoc == cpc(b, doc)
	...
	private void logAffected(List<AffectedAdvicePair> affectRelation) {
		...
		for (AffectedAdvicePair pair : affectRelation) {
			...
			if (pair.isDependent().equals(AffectedAdvicePair.TRIGGERS)) {
				message = "Advice "
						+ pair.getA().getBasicAdvice().getClass()
								.getSimpleName()
						+ " (Pointcut: "
						+ pair.getA().getBasicAdvice().getPointcut().getValue()
						+ ", File: "
						+ adviceContext.getClass().getSimpleName()
						+ ")"
						+ " TRIGGERS Advice "
						+ pair.getB().getBasicAdvice().getClass()
								.getSimpleName() + "(Pointcut: "
	...
	private IAdviceToCheck getLastAdvice() {
	...
	public void addAdvice(IAdviceToCheck advicetocheck) {
	...
	public void setAdviceContext(IAdviceContext ac) {
	...
	private static void printNodeList(NodeList nodelist) throws Exception {
		Node n;
